...
router.get('/', (req, res) => {
    Book.findAll({ include: [{ model: Library, through: LibraryBook, as: 'library_book' }]})
        .then(books => res.json(books))
        .catch(err => res.status(404).json({ nobooksfound: 'Book not found' }));
});

router.get('/:id', (req, res) => {
    Book.findByPk(req.params.id, { include: [{ model: Library, through: LibraryBook, as: 'library_book'}]})
